    ...
    <div class="container max-w-7xl px-4 py-6 antialiased min-h-screen">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl">
            ...
            <div class="flex items-center space-x-6 mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div
                    class="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-2 shadow-md flex-shrink-0 {{ $provincialEducationOffice->active_status ? 'border-green-500' : 'border-red-500' }}">

                    @if ($provincialEducationOffice->logo)
                        <img src="{{ asset('storage/' . $provincialEducationOffice->logo) }}" alt="{{ $provincialEducationOffice->name }}"
                            class="w-full h-full object-cover {{ $provincialEducationOffice->active_status ? '' : 'filter grayscale opacity-60' }}">

                        @if (!$provincialEducationOffice->active_status)
                            <div
                        ...
                        <div class="w-full h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                            <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                ...
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 21h18M4 21V9a2 2 0 012-2h4V5a2 2 0 012-2h4a2 2 0 012 2v2h2a2 2 0 012 2v12M9 21v-6h6v6" />
                            </svg>
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white leading-tight">
                        {{ $provincialEducationOffice->name }}
                    </h1>
                    @if ($provincialEducationOffice->short_name)
                        <p class="text-base text-gray-500 dark:text-gray-400 font-medium mt-1">
                            {{ $provincialEducationOffice->short_name }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @php
                            $basicInfo = [
                                'Workplace ID' => $provincialEducationOffice->workplace_id ?? 'N/A'
                            ];
                        @endphp
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @php
                            $contactInfo = [
                                'Email' => $provincialEducationOffice->email ?? 'N/A',
                                'Phone' => $provincialEducationOffice->phone ?? 'N/A',
                                'Address' => $provincialEducationOffice->address ?? 'N/A',
                                'Postal Code' => $provincialEducationOffice->postal_code ?? 'N/A',
                            ];
                        @endphp
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @php
                            $locationInfo = [
                                'Latitude' => $provincialEducationOffice->latitude ?? 'N/A',
                                'Longitude' => $provincialEducationOffice->longitude ?? 'N/A',
                                'Provincial Education Ministry' => $provincialEducationOffice->provincialMinistryOfEducationOffice->name ?? 'N/A',
