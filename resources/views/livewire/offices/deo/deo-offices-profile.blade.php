    ...
    <div class="container max-w-7xl px-4 py-6 antialiased min-h-screen">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl">
            ...
            <div class="flex items-center space-x-6 mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div
                    class="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-2 shadow-md flex-shrink-0 {{ $divisionalEducationOffice->active_status ? 'border-green-500' : 'border-red-500' }}">

                    @if ($divisionalEducationOffice->logo)
                        <img src="{{ asset('storage/' . $divisionalEducationOffice->logo) }}" alt="{{ $divisionalEducationOffice->name }}"
                            class="w-full h-full object-cover {{ $divisionalEducationOffice->active_status ? '' : 'filter grayscale opacity-60' }}">

                        @if (!$divisionalEducationOffice->active_status)
                            <div
            ...
            <div class="space-y-6">
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @php
                            $basicInfo = [
                                'Workplace ID' => $divisionalEducationOffice->workplace_id
                            ];
                        @endphp
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @php
                            $contactInfo = [
                                'Email' => $divisionalEducationOffice->email ?? 'N/A',
                                'Phone' => $divisionalEducationOffice->phone ?? 'N/A',
                                'Address' => $divisionalEducationOffice->address ?? 'N/A',
                                'Postal Code' => $divisionalEducationOffice->postal_code ?? 'N/A',
                            ];
                        @endphp
                ...
                <section>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-4">
                        @php
                            $locationInfo = [
                                'Administrative District' => $divisionalEducationOffice->district->district_name ?? 'N/A',
                                'Education Zone' => $divisionalEducationOffice->zonalEducationOffice->short_name ?? 'N/A',
                                'Latitude' => $divisionalEducationOffice->latitude ?? 'N/A',
                                'Longitude' => $divisionalEducationOffice->longitude ?? 'N/A',
                            ];
                        @endphp
