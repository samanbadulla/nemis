...
class PrincipalIndex extends Component
{
    ...
    public function mount($id)
    {
        ...
        $this->nic = $this->principal->nic;
        $this->title = $this->principal->title_id;
        $this->fullName = $this->principal->full_name;
        $this->gender = $this->principal->gender_id;
        $this->birthday = Carbon::parse($this->principal->date_of_birth)->format('Y-m-d');
        $this->religion = $this->principal->religion_id;
        $this->ethnicity = $this->principal->ethnicity_id;
        $this->civilStatus = $this->principal->civil_status_id;

        $this->bloodGroup = $this->principal->blood_group_id;
        $this->healthCondition = $this->principal->health_condition;
        $this->healthProblem = $this->principal->health_problem;
    ...
    public function editPersonalInfo()
    {
        ...
        $validated = $this->validate([
            'title' => 'required|string',
            'nic' => ['required', 'string', 'regex:/^(\d{9}[vVxX]|\d{12})$/', new UniqueHashedNic($this->principal->people_id)],
            'fullName' => 'required|string|max:255',
            'gender' => 'required|string',
            'birthday' => 'required|date',
            'religion' => 'required|string',
